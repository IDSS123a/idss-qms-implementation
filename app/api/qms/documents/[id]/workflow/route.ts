...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!can(role, nextStatus === 'approved' || nextStatus === 'published' ? 'approve' : 'edit')) return NextResponse.json({ error: 'Nemate dozvolu za ovu tranziciju.' }, { status: 403 })
  const current = await db.execute(sql`select workspace_id, status from qms_document where id = ${id}::uuid limit 1`)
  ...
  const updated = await db.execute(sql`update qms_document set status = ${nextStatus}, updated_by = ${session.user.id}::uuid, updated_at = now() where id = ${id}::uuid and workspace_id = ${row.workspace_id}::uuid returning id, title, code, status, updated_at`)
  await db.execute(sql`insert into qms_audit_event (workspace_id, user_id, action, entity_type, entity_id, metadata) values (${row.workspace_id}::uuid, ${session.user.id}::uuid, 'workflow_transition', 'document', ${id}::uuid, ${JSON.stringify({ from: row.status, to: nextStatus, reason })}::jsonb)`)
