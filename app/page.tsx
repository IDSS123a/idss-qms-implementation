...
type DocumentRecord = { id: string; title: string; type: string; owner: string; version: string; status: 'Approved' | 'In review' | 'Draft'; date: string; color: 'cyan' | 'yellow' | 'gray' }
...
export default function Page() {
  ...
  const [newType, setNewType] = useState('Procedure')
  const [statusFilter, setStatusFilter] = useState<'All' | DocumentRecord['status']>('All')
  const filtered = useMemo(() => documents.filter((doc) => {
    const matchesQuery = `${doc.id} ${doc.title} ${doc.owner}`.toLowerCase().includes(query.toLowerCase())
    return matchesQuery && (statusFilter === 'All' || doc.status === statusFilter)
  }), [documents, query, statusFilter])
  const createDocument = () => {
    const title = newTitle.trim()
    if (!title) return
    const id = `WI-${String(documents.length + 9).padStart(2, '0')}`
    setDocuments((current) => [{ id, title, type: newType, owner: 'Amina Hadžić', version: '01', status: 'Draft', date: '16 Sep 2026', color: 'gray' }, ...current])
    setNewTitle('')
    setNewType('Procedure')
    setShowCreate(false)
  }
